...
class SimpleMonthView extends View {
    ...
    private static final int SELECTED_CIRCLE_ALPHA = (int) (255);
    private static final int SELECTED_VALID_DAY_ALPHA = 128;
    private static final int SELECTED_BUSY_DAY_ALPHA = 128;
    private static final int TAG_TEXT_ALPHA = (int) (255 * 0.7);
    private static final int SELECTED_RANGE_ALPHA = (int) (255 * 0.3);
    protected static int DEFAULT_HEIGHT = 50;                           // 默认一行的高度
    protected static final int DEFAULT_NUM_ROWS = 6;
    protected static int DAY_SELECTED_RECT_SIZE;                        // 选中圆角矩形半径
    protected static int ROW_SEPARATOR = 24;                            // 每行中间的间距
...
//    private boolean mIsDisplayTag;                                      // 是否显示标签
    ...
    protected Paint mSelectedRangeBgPaint;
    ...
    protected int mTagTextColor;                        // 日期字体颜色
    protected int mSelectedDayTextColor;                // 被选中的日期字体颜色
    protected int mSelectedRangeTextColor;                // 被选中的日期间内的字体颜色
    protected int mPreviousDayTextColor;                // 过去日期的字体颜色
    protected int mAfterDayTextColor;                // 以后日期的字体颜色
    ...
    private final Boolean isAfterDayEnabled;             // 今天以后的日期是否能被操作
    ...
    CalendarDay mStartDate;          // 入住日期
    CalendarDay mEndDate;            // 退房日期

    CalendarDay cellCalendar;        // cell的对应的日期
    private int cellWidth;  // 每一个Cell的宽度
...
//        mDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorDayName, resources.getColor(R.color.normal_day));
        ...
        mTagTextColor = typedArray.getColor(R.styleable.DayPickerView_colorNormalDayText, resources.getColor(R.color.tag_text));
        ...
        mAfterDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorPreviousDayText, resources.getColor(R.color.normal_day));
        ...
        mSelectedRangeTextColor = typedArray.getColor(R.styleable.DayPickerView_colorSelectedRangeText, resources.getColor(R.color.selected_range_text));
...
//        mDrawRect = typedArray.getBoolean(R.styleable.DayPickerView_drawRoundRect, true);
        ...
        isAfterDayEnabled = typedArray.getBoolean(R.styleable.DayPickerView_enableAfterDay, false);
    ...
    private void onDayClick(CalendarDay calendarDay) {
        if (mOnDayClickListener != null && (isPrevDayEnabled || !prevDay(calendarDay.day, today)) &&
                (isAfterDayEnabled || !afterDay(calendarDay.day, today))) {
            mOnDayClickListener.onDayClick(this, calendarDay);
        } else if (!isAfterDayEnabled && afterDay(calendarDay.day, today)) {
            Toast.makeText(getContext(), "不能选择大于今天的日期！", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    protected void drawMonthCell(Canvas canvas) {
        // ?
        int y = MONTH_HEADER_SIZE + ROW_SEPARATOR;
        cellWidth = (mWidth - 2 * mPadding) / mNumDays;
        int paddingDay = cellWidth / 2;
...
//            mTagTextPaint.setColor(mDayTextColor);
            ...
            if (!isAfterDayEnabled && afterDay(day, today)) {
                isAfterDay = true;
                mDayTextPaint.setColor(mAfterDayTextColor);
                mDayTextPaint.setTypeface(Typeface.defaultFromStyle(Typeface.ITALIC));
                canvas.drawText(String.format("%d", day), x, y, mDayTextPaint);
            }
            ...
            if (mStartDate != null && cellCalendar.equals(mStartDate) && !mStartDate.equals(mEndDate)) {
                isBeginDay = true;
                drawDayBg(canvas, x, y, mSelectedDayBgPaint, false);
                mDayTextPaint.setColor(mSelectedDayTextColor);
                canvas.drawText("开始", x, getTagTextYCenter(mDayTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mTagTextPaint);
                if (isToady) {
                    canvas.drawText("今天", x, getTextYCenter(mDayTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
                }
            }
...
//            if (mEndDate != null && cellCalendar.equals(mEndDate) && !mStartDate.equals(mEndDate)) {
            if (mEndDate != null && cellCalendar.equals(mEndDate)) {
                isLastDay = true;
                drawDayBg(canvas, x, y, mSelectedDayBgPaint, false);
                mDayTextPaint.setColor(mSelectedDayTextColor);
                canvas.drawText("结束", x, getTagTextYCenter(mDayTextPaint, y + DAY_SELECTED_RECT_SIZE / 2), mTagTextPaint);
            ...
            if (cellCalendar.after(mStartDate) && cellCalendar.before(mEndDate)) {
                mDayTextPaint.setColor(mSelectedRangeTextColor);
                drawDayBg(canvas, x, y, mSelectedRangeBgPaint, true);
                // 标签变为白色
//                mTagTextPaint.setColor(Color.parseColor("#1C86EE"));
...
//                            x + DAY_SELECTED_RECT_SIZE, (y - MINI_DAY_NUMBER_TEXT_SIZE / 3) + DAY_SELECTED_RECT_SIZE);
                    ...
                    if (mStartDate != null && mEndDate != null && mNearestDay != null &&
                        ...
                        if (mStartDate != null && mEndDate == null && mNearestDay != null && cellCalendar.equals(mNearestDay)) {
                            ...
                            drawDayBg(canvas, x, y, mBusyDayBgPaint, false);
...
//                            x + DAY_SELECTED_RECT_SIZE, (y - MINI_DAY_NUMBER_TEXT_SIZE / 3) + DAY_SELECTED_RECT_SIZE);
                    ...
                    if (mStartDate != null && mEndDate != null && mNearestDay != null &&
                        ...
                        if (mStartDate != null && mEndDate == null && mNearestDay != null && cellCalendar.equals(mNearestDay)) {
                            ...
                            drawDayBg(canvas, x, y, mInValidDayBgPaint, false);
//                            canvas.drawRoundRect(rectF, 10.0f, 10.0f, mBusyDayBgPaint);
...
//            }
            ...
            // 绘制日期
            if (!isToady && !isPrevDay && !isAfterDay && !isInvalidDays && !isBusyDay) {
                canvas.drawText(String.format("%d", day), x, getTextYCenter(mTagTextPaint, y - DAY_SELECTED_RECT_SIZE / 2), mDayTextPaint);
            }
...
//        canvas.drawLine(mPadding * 3, y + mRowHeight / 2, mWidth - mPadding * 3, y + mRowHeight / 2, mDayTextPaint);
    }
...
//        mTitleBGPaint.setStyle(Style.FILL);
        ...
        // 被选中的日期间的背景paint
        mSelectedRangeBgPaint = new Paint();
        mSelectedRangeBgPaint.setFakeBoldText(true);
        mSelectedRangeBgPaint.setAntiAlias(true);
        mSelectedRangeBgPaint.setColor(mSelectedDaysBgColor);
        mSelectedRangeBgPaint.setTextAlign(Align.CENTER);
        mSelectedRangeBgPaint.setStyle(Style.FILL);
        mSelectedRangeBgPaint.setAlpha(SELECTED_RANGE_ALPHA);
        ...
        mBusyDayBgPaint.setAlpha(SELECTED_BUSY_DAY_ALPHA);
        ...
        mInValidDayBgPaint.setAlpha(SELECTED_VALID_DAY_ALPHA);
...
//        mSelectedDayTextPaint.setFakeBoldText(true);
        ...
        mTagTextPaint.setAlpha(TAG_TEXT_ALPHA);
    }
...
//        }
        ...
        if (params.containsKey("mNearestDay")) {
            mNearestDay = (CalendarDay) params.get("mNearestDay");
        }
     ...
     * @param y
     */
    private void drawDayBg(Canvas canvas, int x, int y, Paint paint, boolean isRangeDay) {
        RectF rectF = new RectF(x - cellWidth / 2, y - cellWidth / 2,
    ...
    private float getTextYCenter(Paint paint, int y) {
        ...
        float offY = fontTotalHeight / 2 - fontMetrics.descent;
        return y + offY * 2;
    }
     ...
     * @return
     */
    private float getTagTextYCenter(Paint paint, int y) {
        Paint.FontMetrics fontMetrics = paint.getFontMetrics();
        float fontTotalHeight = fontMetrics.bottom - fontMetrics.top;
        float offY = fontTotalHeight / 2 - fontMetrics.descent;
        return y + offY / 2;
